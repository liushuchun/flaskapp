var utoken, uid;
var host = 'http://app.fengqiaoju.com/';
...
var user = {
	...
	'updateQQ': function(qq, obj) {
		var url = host + '?m=user&a=updateQQ';
		var uid = this.uid();
		ajax.post(url, {
			uid: uid,
			utoken: this.utoken(),
	...
	'register': function(username, password, mobile, qq, sex, addr, obj) {
		var url = host + '?m=user&a=register';
	...
	'sendMobile': function(mobile, obj) {
		var url = host + '?m=validation&a=sendMobile';
		ajax.get(url, {
			mobile: mobile
		}, obj);
	},
	...
	'checkMobile': function(mobile, code, obj) {
		if (mobile.length != 11) {
			alert('请输入11位手机号码');
			return false;
		}
		...
		var url = host + '?m=validation&a=checkMobile';
		ajax.get(url, {
			mobile: mobile,
			code: code
		}, obj);
